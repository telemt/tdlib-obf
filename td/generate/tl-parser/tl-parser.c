...
int expect(char *s) {
  if (!parse.lex.ptr || parse.lex.ptr == (void *)-1 || parse.lex.type == lex_error || parse.lex.type == lex_none ||
      parse.lex.len != (int)strlen(s) || memcmp(s, parse.lex.ptr, parse.lex.len)) {
...
#define PARSE_INIT(_type)           \
  struct parse save = save_parse(); \
  struct tree *T = tree_alloc();    \
  T->type = (_type);                \
  T->lex_line = parse.line;         \
  T->lex_line_pos = parse.line_pos; \
  struct tree *S __attribute__((unused));
#define PARSE_FAIL  \
  load_parse(save); \
  tree_delete(T);   \
  return 0;
#define PARSE_OK return T;
#define PARSE_TRY_PES(x) \
  if (!(S = x())) {      \
    PARSE_FAIL;          \
  }                      \
  { tree_add_child(T, S); }
#define PARSE_TRY_OPT(x)  \
  if ((S = x())) {        \
    tree_add_child(T, S); \
    PARSE_OK              \
  }
#define PARSE_TRY(x) S = x();
#define PARSE_ADD(_type) \
  S = tree_alloc();      \
  S->type = _type;       \
  tree_add_child(T, S);
#define EXPECT(s)      \
...
void tl_set_var_value_num(struct tree_var_value **T, struct tl_combinator_tree *var, struct tl_combinator_tree *value,
                          long long num_value) {
    ...
    case act_type:
      ...
      if (T->flags & 2) {
        ...
        if (T->flags & 4) {
          ...
          tl_buf_add_string_q(t->constructors[0]->real_id ? t->constructors[0]->real_id : t->constructors[0]->id, -1,
                              x);
        } else {
...
int _tl_finish_subtree(struct tl_combinator_tree *R, int x, long long y) {
  ...
  assert((R->right->type == type_type && R->right->type_len == 0) || R->right->type == type_num ||
         R->right->type == type_num_value);
...
struct tl_combinator_tree *tl_parse_opt_args(struct tree *T) {
  ...
  for (i = 0; i <= T->nc - 2; i++) {
    ...
    S->type_len = 0;
    S->act = act_field;
    S->data = mystrdup(T->c[i]->text, T->c[i]->len);
    assert(S->data);
  ...
  if (tl_get_var(T->c[0]->text, T->c[0]->len)) {
    ...
    for (i = 1; i < T->nc; i++) {
      ...
      assert(L->right->type == type_num || L->right->type == type_num_value ||
             (L->right->type == type_type && L->right->type_len == 0));
    }
  }
...
int tl_parse_combinator_decl(struct tree *T, int fun) {
  ...
  struct tl_constructor *c =
      !fun ? tl_add_constructor(t, T->c[0]->text, T->c[0]->len, 0) : tl_add_function(t, T->c[0]->text, T->c[0]->len, 0);
  if (!c) {
    TL_FAIL;
  }
  c->left = L;
...
int tl_parse_partial_type_app_decl(struct tree *T) {
  ...
  for (i = 0; i < t->constructors_num; i++) {
    ...
    if (!check_constructors_equal(B, L, &V)) {
      continue;
    }
    B = reduce_type(B, nt);
...
int tl_parse_partial_comb_app_decl(struct tree *T, int fun) {
  ...
  struct tl_constructor *c =
      !fun ? tl_get_constructor(T->c[0]->text, T->c[0]->len) : tl_get_function(T->c[0]->text, T->c[0]->len);
  if (!c) {
  ...
  struct tl_constructor *r =
      !fun ? tl_add_constructor(c->type, _buf, strlen(_buf), 1) : tl_add_function(c->type, _buf, strlen(_buf), 1);
  r->left = L;
  r->right = R;
  snprintf(_buf, 100000, "%s", c->id);
  r->real_id = tstrdup(_buf);
...
void write_args(struct tl_combinator_tree *T, struct tree_var_value **v, int *last_var) {
  ...
  if (T->left->act == act_type && T->left->data &&
      (!strcmp(((struct tl_type *)T->left->data)->id, "#") || !strcmp(((struct tl_type *)T->left->data)->id, "Type"))) {
