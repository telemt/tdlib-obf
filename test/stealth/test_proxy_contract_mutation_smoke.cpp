...
TEST(ProxyContractMutationSmoke, MutantM1_RawIpMustUseEffectiveTransportTypeOnly) {
  ...
  // M1 mutant: replacing effective_transport_type with transport_type must be detected.
  ASSERT_TRUE(normalized.find("RawConnection::create(ip_address,std::move(connection_data.buffered_socket_fd),"
                              "transport_type,nullptr)") == td::string::npos);
  ASSERT_TRUE(normalized.find("prepare_connection(route.socket_ip_address,std::move(socket_fd),proxy,"
                              "route.mtproto_ip_address,transport_type,\"Raw\"") == td::string::npos);
}

TEST(ProxyContractMutationSmoke, MutantM2_RawIpMustDialResolvedRouteSocketAddress) {
  auto source = td::mtproto::test::read_repo_text_file("td/telegram/net/ConnectionCreator.cpp");
  auto raw_ip_request = extract_source_region(source, "void ConnectionCreator::request_raw_connection_by_ip(",
                                              "Result<mtproto::TransportType> ConnectionCreator::get_transport_type(");
  auto normalized = normalize_for_contract(raw_ip_request);
  ...
  // M2 mutant: direct dial/open using ip_address must not appear.
  ASSERT_TRUE(normalized.find("SocketFd::open(ip_address)") == td::string::npos);
  ASSERT_TRUE(normalized.find("prepare_connection(ip_address,std::move(socket_fd),proxy,") == td::string::npos);
}

TEST(ProxyContractMutationSmoke, MutantM3_PingProxyMustResolveEffectiveProxyBeforeDirectBranch) {
  auto source = td::mtproto::test::read_repo_text_file("td/telegram/net/ConnectionCreator.cpp");
  auto ping_proxy = extract_source_region(source, "void ConnectionCreator::ping_proxy(",
                                          "void ConnectionCreator::ping_proxy_resolved(");
  auto normalized = normalize_for_contract(ping_proxy);

  auto resolve_pos = normalized.find("autoproxy=resolve_effective_ping_proxy(active_proxy,requested_proxy.get());");
  auto direct_branch_pos = normalized.find("if(!proxy.use_proxy())");
  ...
  // M3 mutant: bypassing resolver and assigning active_proxy directly must not appear.
  ASSERT_TRUE(normalized.find("autoproxy=active_proxy;") == td::string::npos);
}
