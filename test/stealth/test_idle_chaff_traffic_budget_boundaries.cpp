...
StealthConfig make_boundary_config() {
  MockRng rng(1);
...
struct Harness final {
  ...
  static Harness create(StealthConfig config, td::uint64 seed = 77) {
    Harness harness;
...
TEST(IdleChaffTrafficBudgetBoundaries, BudgetResumesExactlyAtOldestSampleExpiry) {
...
TEST(IdleChaffTrafficBudgetBoundaries, PendingRealWriteSuppressesBudgetReadyChaffUntilDrain) {
  ...
  const auto blocked_wakeup = harness.next_wakeup();
  harness.flush_at(blocked_wakeup - 0.100);
  harness.queue_real_write(td::Slice("user-data"));

  ASSERT_TRUE(harness.next_wakeup() <= harness.clock->now() + 1e-6);

  harness.flush_at(blocked_wakeup);
  ASSERT_EQ(3, harness.inner->write_calls);
  ASSERT_EQ(td::string("user-data"), harness.inner->written_payloads.back());

  const auto rearmed_wakeup = harness.next_wakeup();
