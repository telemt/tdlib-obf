...
TEST(ConnectionCreatorPingProxyLogSourceContract, PingProbeFailureIncludesRetryPolicySummaryAndTransportContext) {
  ...
  ASSERT_TRUE(probe_region.find("auto transport_type_for_log = transport_type;") != td::string::npos);
  ASSERT_TRUE(probe_region.find("tag(\"transport\", raw_ip_transport_name(transport_type_for_log))") !=
              td::string::npos);
  ASSERT_TRUE(probe_region.find("tag(\"tls_emulation\", transport_type_for_log.secret.emulate_tls())") !=
              td::string::npos);
  ASSERT_TRUE(probe_region.find("raw_ip_transport_name(transport_type)") == td::string::npos);
  ASSERT_TRUE(probe_region.find("transport_type.secret.emulate_tls()") == td::string::npos);
