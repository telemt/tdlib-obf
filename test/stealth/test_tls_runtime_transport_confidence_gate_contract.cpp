...
TEST(TlsRuntimeTransportConfidenceGateContract, UnknownConfidenceNeverSelectsTransportStrongProfileClass) {
  ...
  for (td::uint32 idx = 0; idx < 4096; idx++) {
    auto unix_time = static_cast<td::int32>(1712345678 + idx * 31);
    td::string domain = "transport-unknown-contract-" + td::to_string(idx) + ".example";
    auto profile = pick_runtime_profile(domain, unix_time, linux_platform);
    auto metadata = profile_fixture_metadata(profile);
    ASSERT_TRUE(metadata.transport_claim_level == TransportClaimLevel::TlsOnly);
  }
}
